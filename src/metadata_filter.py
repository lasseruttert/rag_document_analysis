...
if __name__ == '__main__':
    ...
    sample_docs = [
        ...
        {
            'content': 'Docker configuration example...',
            'metadata': {
                'filename': 'docker_config.docx',
                'file_type': 'docx',
                'chunk_size': 600,
                'position': 200,
                'semantic_density': 0.45,
                'keyword_tags': ['docker', 'config', 'example'],
                'contains_heading': True
            }
        }
    ]
    ...
    print("\n1. Testing file type filtering...")
    pdf_filter = MetadataFilter.by_file_type("pdf")
    pdf_docs = MetadataFilter.apply_filters(sample_docs, pdf_filter)
    print(f"PDF documents: {len(pdf_docs)} found")
    
    # Test 2: Size filtering
    print("\n2. Testing content size filtering...")
    large_docs_filter = MetadataFilter.by_content_size(min_size=800)
    large_docs = MetadataFilter.apply_filters(sample_docs, large_docs_filter)
    print(f"Large documents (>=800 chars): {len(large_docs)} found")
    
    # Test 3: Combined filtering
    print("\n3. Testing combined filtering...")
    ...
    print(f"TXT/PDF documents with high semantic density: {len(filtered_docs)} found")
    
    # Test 4: Keyword filtering
    print("\n4. Testing keyword filtering...")
    keyword_filter = MetadataFilter.by_keywords(['kubernetes', 'python'])
    keyword_docs = MetadataFilter.apply_filters(sample_docs, keyword_filter)
    print(f"Documents containing 'kubernetes' or 'python': {len(keyword_docs)} found")
    
    # Test 5: Heading presence
    print("\n5. Testing heading presence filtering...")
    heading_filter = MetadataFilter.by_heading_presence(True)
    heading_docs = MetadataFilter.apply_filters(sample_docs, heading_filter)
    print(f"Documents with headings: {len(heading_docs)} found")
    
    print("\nMetadata Filtering System test completed!")